...
module.exports = {
    getProduct: async (req, res) => {
        const { id } = req.params;
        if (!id) return res.status(400).json({ error: 'No se proporciono ningun id' });
        console.log(id)
    ...
    createProduct: async (req, res) => {
        ...
        if (!name || !description || !quantity || !price || !stock || !images || !category) return res.status(400).json({ error: 'Debe ingresar todos los parametros solicitados' });
        const product = await Product.findOne({ name });
        if (product) return res.status(400).json({ error: 'El nombre de el producto ya existe' });
